...
2. **Rebuild**: `scripts/extract_content.py` (Python, BeautifulSoup) parses the mirror and writes Hugo content bundles under `content/`. Safe to re-run in place if the mirror is refreshed: it skips any `.md` file or image that already exists (pass `--force` to overwrite `.md` files deliberately), so the hand-authored translations, in-place edits (alt text, title fixes), and refetched originals all survive. Never wipe `content/` first (see `CLAUDE.md`).
